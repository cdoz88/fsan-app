...
function compile() {
  ...
  const parsePositionFile = (filename, position, fromLine, rowCallback) => {
    ...
    // 🛡️ THE FIX: This function catches duplicate headers (like YDS and YDS) 
    // and correctly renames the second one to YDS_2 so data doesn't get overwritten!
  ...
  parsePositionFile('QB 2026 Proj Stats.csv', 'QB', 2, (row, obj) => {
    obj.pass_yds = cleanNum(row.YDS);       // First YDS column (Passing)
    obj.pass_tds = cleanNum(row.TDS);
    obj.ints = cleanNum(row.INTS);
    obj.rush_yds = cleanNum(row.YDS_2);     // Second YDS column (Rushing)
    obj.rush_tds = cleanNum(row.TDS_2);     // Second TDS column (Rushing)
    obj.fumbles = cleanNum(row.FL);
  });

  // 2. RBs
  parsePositionFile('RB 2026 Proj Stats.csv', 'RB', 2, (row, obj) => {
    obj.rush_yds = cleanNum(row.YDS);       // First YDS column (Rushing)
    obj.rush_tds = cleanNum(row.TDS);
    obj.receptions = cleanNum(row.REC);
    obj.rec_yds = cleanNum(row.YDS_2);      // Second YDS column (Receiving)
  ...
  parsePositionFile('WR 2026 Proj Stats.csv', 'WR', 2, (row, obj) => {
    obj.receptions = cleanNum(row.REC);
    obj.rec_yds = cleanNum(row.YDS);        // First YDS column (Receiving)
  ...
  parsePositionFile('TE 2026 Proj Stats.csv', 'TE', 2, (row, obj) => {
    obj.receptions = cleanNum(row.REC);
    obj.rec_yds = cleanNum(row.YDS);        // TEs only have one YDS column
    obj.rec_tds = cleanNum(row.TDS);
    obj.fumbles = cleanNum(row.FL);
  });
