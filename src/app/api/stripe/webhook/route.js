...
export async function POST(req) {
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // 🚀 LOGIC 2: Handle Charity Donations
      if (donationAmount > 0) {
        const donationQuery = `
          mutation AddDnoDonation {
            addDnoDonation(
              input: {
                userId: ${wpUserId},
        ...
        const donRes = await fetch('https://admin.fsan.com/graphql', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ query: donationQuery }),
        });
        
        const donJson = await donRes.json();
        
        if (donJson.errors) {
          console.error("WordPress failed to add DNO donation:", donJson.errors);
        } else {
          console.log(`Successfully added $${donationAmount} DNO donation for WP User ${wpUserId}`);
        }
      }

      // 🚀 LOGIC 3: Handle Subscription Upgrades (Includes sending the Source)
      if (purchaseType === 'subscription' || purchaseType === 'dno_ticket_bundle') {
        ...
        if (wpJson.errors) {
          ...
          console.log(`Successfully upgraded WP User ${wpUserId} to ${assignedTier} from source ${purchaseType}`);
        }
      }
    }
