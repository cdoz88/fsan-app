...
import { User, Mail, Lock, Loader2, CreditCard, ShieldCheck, CheckCircle2, FileText, ShoppingCart, Tag, AlertTriangle, ShieldAlert, Book, Download, Shirt, Users, Settings, Gift, LogOut, ChevronRight, Image as ImageIcon, Zap, Star, Ticket, Link as LinkIcon, RefreshCw, X, Trophy } from 'lucide-react';
...
export default function AccountClient() {
  ...
  // 🚀 NEW: State for Sync Sports Submenu
  ...
  const handleVerifySleeper = async (e) => {
    e.preventDefault();
  ...
  const renderTabContent = () => {
    return (
      <div className="bg-[#111] rounded-3xl border border-gray-800 p-6 md:p-10 shadow-2xl relative overflow-hidden min-h-[400px]">
        ...
        {/* 🚀 NEW: Synced Leagues Tab (With Submenu & Verification & List & Remove) */}
        {activeTab === 'Synced Leagues' && (
          <div className="space-y-8 animate-in fade-in duration-500 relative z-10">
            ...
            {/* 🚀 NEW: Submenu for Sports */}
            <div className="flex gap-2 border-b border-gray-800 mb-6">
              <button 
                onClick={() => setSyncSportTab('football')} 
                className={`px-4 py-3 text-xs font-bold uppercase tracking-widest border-b-2 transition-colors ${syncSportTab === 'football' ? 'border-red-500 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
              >
                Football
              </button>
              <button 
                onClick={() => setSyncSportTab('basketball')} 
                className={`px-4 py-3 text-xs font-bold uppercase tracking-widest border-b-2 transition-colors ${syncSportTab === 'basketball' ? 'border-orange-500 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
              >
                Basketball
              </button>
              <button 
                onClick={() => setSyncSportTab('baseball')} 
                className={`px-4 py-3 text-xs font-bold uppercase tracking-widest border-b-2 transition-colors ${syncSportTab === 'baseball' ? 'border-blue-500 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
              >
                Baseball
              </button>
            </div>
            ...
            <div className="space-y-6 relative z-10">
              {syncSportTab === 'football' ? (
                <>
                    ...
                    {userTier === 'pro' || userTier === 'pro-plus' ? (
                      <div className="flex flex-col gap-4">
                        ...
                        {!verifiedSleeperUser ? (
                          <div className="flex flex-col gap-2">
                            <div className="flex gap-2">
                              <input 
                                ...
                                className="flex-1 bg-[#111] border border-gray-700 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors shadow-inner"
                              />
                              <button 
                                onClick={handleVerifySleeper}
                                disabled={isVerifyingSleeper}
                                className="bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm px-6 py-2.5 rounded-xl transition-all shadow-sm disabled:opacity-50 flex items-center gap-2 min-w-[100px] justify-center"
                              >
                                {isVerifyingSleeper ? <Loader2 size={16} className="animate-spin" /> : 'Verify'}
                              </button>
                            </div>
                            {syncStatus.message && (
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-[#111] p-4 rounded-xl border border-gray-700 shadow-inner gap-4">
                            <div className="flex items-center gap-4">
                              {verifiedSleeperUser.avatar ? (
                                ...
                                <div className="w-10 h-10 rounded-full bg-gray-800 border border-gray-600 flex items-center justify-center shrink-0">
                                   <User size={18} className="text-gray-400" />
                                </div>
                              )}
                              <div>
                                <div className="text-sm font-bold text-white">{verifiedSleeperUser.username}</div>
                                <div className="text-[10px] text-green-400 font-bold uppercase tracking-widest flex items-center gap-1"><CheckCircle2 size={10}/> Verified</div>
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              <button onClick={() => setVerifiedSleeperUser(null)} className="text-xs font-bold text-gray-400 hover:text-white px-3 py-2 transition-colors uppercase tracking-wider">Cancel</button>
                              <button onClick={handleSleeperSync} disabled={isSyncing} className="bg-green-600 hover:bg-green-500 text-white font-bold text-xs uppercase tracking-widest px-5 py-2.5 rounded-lg transition-all shadow-sm disabled:opacity-50 flex items-center gap-2">
                                {isSyncing ? <RefreshCw size={14} className="animate-spin" /> : 'Confirm & Sync'}
                              </button>
                            </div>
                          </div>
                        )}
                            ...
                            <div className="space-y-3">
                              {allLeagues.filter(l => l.platform === 'sleeper').map(league => (
                                <div key={league.id} className="flex items-center justify-between bg-[#111] border border-gray-800 p-3.5 rounded-xl hover:border-gray-700 transition-colors group">
                                   <div className="flex items-center gap-4">
                                      {league.avatar ? (
                                        <img src={`https://sleepercdn.com/avatars/past/${league.avatar}`} className="w-9 h-9 rounded-full border border-gray-700 shadow-sm" alt="League" />
                                      ) : (
                                   ...
                                   <button 
                                     ...
                                     title="Remove League"
                                     className="text-gray-500 hover:text-red-500 hover:bg-red-900/20 p-2 rounded-lg transition-all border border-transparent hover:border-red-900/50"
                                   >
                                      <X size={16} />
                                   </button>
                                </div>
                  ...
                  {/* Yahoo & ESPN Placeholders */}
                  <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl p-5 opacity-50 grayscale pointer-events-none">
                    <div className="flex items-center gap-3">
        ...
        {activeTab === 'My Perks' && (
          <div className="space-y-6 animate-in fade-in duration-500 relative z-10">
            ...
            {/* FOMO UPGRADE BOX (Only shows for free users) */}
            {userTier === 'free' && (
              <div className="p-[3px] rounded-3xl bg-[conic-gradient(from_225deg_at_50%_50%,#1b75bb_0%,#c30b16_25%,#c30b16_50%,#f5a623_75%,#1b75bb_100%)] shadow-[0_10px_40px_rgba(195,11,22,0.15)] mb-10">
                <div className="bg-[#111] rounded-[21px] p-8 md:p-12 text-center flex flex-col items-center justify-center">
                  ...
                  <Link 
                    href="/subscribe" 
                    className="px-10 py-4 bg-gradient-to-r from-[#e42d38] to-[#8a1a20] hover:from-[#f03a45] hover:to-[#a3222a] text-white font-black text-lg uppercase tracking-widest rounded-xl transition-transform hover:scale-105 shadow-[0_10px_20px_rgba(228,45,56,0.3)] border border-[#e42d38]/50"
                  >
                    Upgrade to Pro+
                  </Link>
                </div>
            ...
            {/* PERKS GRID (Visible to everyone, actions conditional) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              
              {/* Football Rookie Draft Guide Card */}
              ...
              <div className="bg-gradient-to-br from-[#1a1a1a] to-[#111] border border-gray-800 rounded-2xl p-6 relative overflow-hidden group hover:border-gray-600 transition-all shadow-lg flex flex-col h-full md:col-span-2">
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                      ...
                      <p className="text-xs text-gray-400 leading-relaxed mb-6 flex-1 pr-4 max-w-3xl">
