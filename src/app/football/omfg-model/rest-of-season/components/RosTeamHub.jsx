...
const CustomXAxisTick = ({ x, y, payload, alphaData, playerDB }) => {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <clipPath id={clipId}><circle cx="0" cy="15" r="16" /></clipPath>
      <circle cx="0" cy="15" r="16" fill="#1a1a1a" stroke="#333" strokeWidth="1" />
      <image href={headshotUrl} x="-16" y="-1" height="32" width="32" clipPath={`url(#${clipId})`} onError={(e) => { e.target.setAttribute('href', 'https://sleepercdn.com/images/v2/icons/player_default.webp'); }} />
      <text x="0" y="45" dy={0} textAnchor="middle" fill="#888" fontSize="10px" fontWeight="bold">{lastName}</text>
    </g>
  );
};
...
function TeamDropdown({ options, value, onChange }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button onClick={() => setIsOpen(!isOpen)} className="bg-[#1a1a1a] border border-gray-700 text-white font-black text-sm uppercase tracking-widest rounded-xl py-3 pl-4 pr-10 flex items-center justify-between gap-3 shadow-xl hover:border-gray-500 transition-colors w-56 md:w-64">
        <div className="flex items-center gap-3">
          {value && (
            <img src={`https://sleepercdn.com/images/team_logos/nfl/${value.toLowerCase()}.png`} alt={value} className="w-6 h-6 object-contain drop-shadow-md" onError={(e) => e.target.style.display = 'none'} />
          )}
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-56 md:w-64 bg-[#1a1a1a] border border-gray-700 rounded-xl shadow-2xl py-2 z-[120] max-h-72 overflow-y-auto scrollbar-hide animate-in fade-in zoom-in-95 duration-150">
          {options.map((team) => (
            <button key={team} onClick={() => { onChange(team); setIsOpen(false); }} className={`w-full text-left px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-colors flex items-center gap-3 ${value === team ? 'bg-red-600/20 text-red-500 border-l-2 border-red-500' : 'text-gray-300 hover:bg-[#252525] hover:text-white'}`}>
              <img src={`https://sleepercdn.com/images/team_logos/nfl/${team.toLowerCase()}.png`} alt={team} className="w-5 h-5 object-contain" onError={(e) => e.target.style.display = 'none'} />
              {team}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 mb-3 border-b border-gray-800 pb-3">
          {!isOther && (
            <div className="w-10 h-10 rounded-full bg-gray-800 border-2 overflow-hidden shrink-0" style={{ borderColor: data.fill }}>
              <img src={headshotUrl} alt={data.name} className="w-full h-full object-cover pt-1" onError={(e) => { e.target.src = 'https://sleepercdn.com/images/v2/icons/player_default.webp'; }} />
            </div>
          )}
          <div>
...
export default function RosTeamHub({ visibleData, isHistorical, isSyncing }) {
  ...
  const alphaData = useMemo(() => {
    ...
    return teamPlayers
      .map(p => ({
        ...
        fullName: p.Player,
        pos: p.Position,
        omfg: Number(p['Preseason OMFG'] ?? p['In-Season OMFG Score'] ?? p['OMFG Score']) || 0,
  ...
  const metricLabels = { 'First Read Targets': 'First Read', 'End Zone Targets': 'End Zone', 'Rush Attempts': 'Rush' };
  ...
  if (isSyncing || dbLoading) {
    return (
      <div className="bg-[#111] rounded-2xl shadow-2xl border border-gray-800 min-h-[500px] flex flex-col items-center justify-center text-red-600">
        ...
        <h3 className="text-sm font-black text-white uppercase tracking-wider">Syncing Team Data...</h3>
      </div>
    );
  }
    ...
    <div className="bg-[#111] rounded-2xl shadow-2xl border border-gray-800 animate-in fade-in duration-500 relative min-h-[500px] p-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-800/50">
        <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
          <TeamDropdown options={availableTeams} value={selectedTeam} onChange={setSelectedTeam} />
          <div className="flex flex-wrap bg-[#1a1a1a] p-1.5 rounded-2xl shadow-inner border border-gray-800 w-fit shrink-0">
            {positions.map(pos => (
              <button key={pos} onClick={() => setTeamPosition(pos)} className={`px-3 py-1.5 rounded-xl font-bold text-[10px] uppercase tracking-widest transition-all ${teamPosition === pos ? 'bg-red-600 text-white shadow-[0_0_15px_rgba(220,38,38,0.4)]' : 'text-gray-500 hover:text-white hover:bg-[#252525]'}`}>{pos}</button>
            ))}
          </div>
        </div>
        <div className="text-right hidden xl:flex items-center gap-3">
          <div className="relative group cursor-help">
            ...
            <div className="absolute bottom-full right-0 mb-3 px-4 py-3 bg-[#1a1a1a] border border-gray-700 text-gray-300 text-[11px] rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[100] w-64 text-center pointer-events-none normal-case tracking-normal font-medium leading-relaxed">
              Opportunity tells us how many chances a player receives. This tool visualizes how an offense distributes its high-value touches like first reads and end zone targets.
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-[#151515] border border-gray-800 rounded-2xl p-5 shadow-inner flex flex-col">
          <div className="flex items-center gap-2 mb-6">
            <div className="w-8 h-8 rounded-full bg-gray-800 text-gray-400 flex items-center justify-center border border-gray-700"><Target size={16} /></div>
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-widest leading-none">Top RoS Utilization</h3>
              <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest mt-1">Highest RoS OMFG Scores</p>
            </div>
          </div>
            ...
            {alphaData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={alphaData} margin={{ top: 20, right: 0, left: -20, bottom: 40 }}>
                  ...
                  <XAxis dataKey="fullName" axisLine={false} tickLine={false} tick={<CustomXAxisTick alphaData={alphaData} playerDB={playerDB} />} interval={0} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#555', fontSize: 10, fontWeight: 'bold' }} />
                  <RechartsTooltip content={<AlphaTooltip />} cursor={{ fill: '#222' }} />
                  <Bar dataKey="omfg" fill={activeColor} radius={[6, 6, 0, 0]} animationDuration={1000}>
                    {alphaData.map((entry, index) => (<Cell key={`cell-${index}`} fill={index === 0 ? activeColor : hexToRgba(activeColor, 0.55)} />))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ) : (<div className="absolute inset-0 flex items-center justify-center text-gray-500 text-xs font-bold uppercase">No Players Found</div>)}
          </div>
        </div>
        ...
        <div className="bg-[#151515] border border-gray-800 rounded-2xl p-5 shadow-inner flex flex-col">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-2">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-gray-800 text-gray-400 flex items-center justify-center border border-gray-700 shrink-0"><Crosshair size={16} /></div>
              <div>
            ...
            <div className="flex bg-[#111] p-1 rounded-xl border border-gray-800 w-fit shrink-0 relative z-50">
              {['First Read Targets', 'End Zone Targets', 'Rush Attempts'].map(metric => (
                <button key={metric} onClick={() => setDonutMetric(metric)} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-colors ${donutMetric === metric ? 'bg-gray-800 text-white shadow-md' : 'text-gray-500 hover:text-white'}`}>{metricLabels[metric]}</button>
              ))}
            </div>
          </div>
          <div className="flex-1 w-full min-h-[300px] relative flex items-center justify-center">
            ...
            {donutData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%" className="relative z-10">
                <PieChart>
                  <Pie data={donutData} cx="50%" cy="50%" innerRadius={70} outerRadius={110} paddingAngle={3} dataKey="value" stroke="none" animationDuration={1000}>
                    {donutData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.fill} className="hover:opacity-80 transition-opacity" style={{ cursor: 'pointer' }} />))}
                  </Pie>
