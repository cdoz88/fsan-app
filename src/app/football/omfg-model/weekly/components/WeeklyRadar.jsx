      ...
      <div className="bg-[#1a1a1a] border border-gray-700 p-4 rounded-xl shadow-2xl z-[100] min-w-[180px]">
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between gap-6 text-xs">
            <span className="text-gray-400 uppercase tracking-widest font-bold">Matchup:</span>
            <span className="text-white font-black">{data.matchup.toFixed(1)}</span>
          </div>
          <div className="flex justify-between gap-6 text-xs">
            <span className="text-gray-400 uppercase tracking-widest font-bold">Proj Pts:</span>
...
export default function WeeklyRadar({ visibleData, isHistorical, isSyncing }) {
  ...
  const radarChartData = useMemo(() => {
    ...
    if (validData.length === 0) return { data: [], medMatchup: 0, medPpg: 0 };

    const matchupList = validData.map(p => Number(p['Matchup Score'] ?? p['Opportunity Factor'] ?? 50));
    const ppgList = validData.map(p => isHistorical ? Number(p['Actual Fantasy Points']) : Number(p['Projected Fantasy Points']));

    const medMatchup = getMedian(matchupList);
    ...
    const data = validData.map(p => {
      ...
      if (matchup > medMatchup && ppg < medPpg) {
        fill = '#10b981'; 
        label = 'Great Matchup / Sleeper';
        colorClass = 'text-emerald-400';
      } else if (matchup < medMatchup && ppg > medPpg) {
        fill = '#ef4444'; 
        label = 'Bad Matchup / Fade';
        colorClass = 'text-red-400';
      } else if (matchup > medMatchup && ppg > medPpg) {
        fill = '#3b82f6'; 
        label = 'Smash Play / Elite';
    ...
    <div className="bg-[#111] rounded-2xl shadow-2xl border border-gray-800 animate-in fade-in duration-500 relative min-h-[540px] p-6 pt-10">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 relative z-20">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <div className="relative group cursor-help flex items-center justify-center">
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-4 py-3 bg-[#1a1a1a] border border-gray-700 text-gray-300 text-[11px] rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[100] w-64 text-center pointer-events-none normal-case tracking-normal font-medium leading-relaxed">
              Plots Matchup Score against Projected Points. Look for 'Buy Low' sleepers in the bottom right (great matchup, lower projected points due to past box score variance) and 'Fade' candidates in the top left.
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-[#1a1a1a] px-3 py-1.5 rounded-xl border border-gray-800 shadow-xl">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-300">Great Matchup</span>
          </div>
          <div className="flex items-center gap-2 bg-[#1a1a1a] px-3 py-1.5 rounded-xl border border-gray-800 shadow-xl">
            <div className="w-3 h-3 rounded-full bg-red-500"></div>
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-300">Fade / Trap</span>
          </div>
        </div>
      </div>
      ...
      {isSyncing ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-red-600 z-10 bg-[#111]/80 backdrop-blur-sm rounded-2xl">
          ...
          <h3 className="text-sm font-black text-white uppercase tracking-wider mb-2">Calculating Weekly Medians...</h3>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 20, right: 30, bottom: 25, left: 20 }}>
              ...
              <XAxis 
                ...
                label={{ 
                  value: 'OPPONENT DIFFICULTY (MATCHUP SCORE)', 
              ...
              <ReferenceLine x={radarChartData.medMatchup} stroke="#666" strokeWidth={2} strokeDasharray="4 4" />
