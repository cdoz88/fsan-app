...
import { Ticket, MonitorSmartphone, MapPin, Calendar, Lock, Loader2, CheckCircle2, AlertCircle, ExternalLink, Trophy, Shield, Users, Coins, UserCheck, BookOpen, Handshake, Mail, Medal, Gift, ListOrdered, Clock, LogOut, X, Star, Shirt, HeartHandshake, SlidersHorizontal } from 'lucide-react';
...
function SuccessToast({ setActiveTab, setDraftView, loadDnoPool, isAuthed }) {
  ...
  useEffect(() => {
    if (searchParams?.get('checkout') === 'success') {
      ...
      // 1. Force the user to the Drafts tab -> Online view where their tickets are
      setActiveTab('drafts');
      setDraftView('online');
...
export default function DraftNightOutClient({ proToolsMenu, connectMenu, initialLeaderboard }) {
  ...
  const [draftView, setDraftView] = useState('online'); // 'online' | 'live'
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>
        <SuccessToast setActiveTab={setActiveTab} setDraftView={setDraftView} loadDnoPool={loadDnoPool} isAuthed={isAuthed} />
      </Suspense>
        ...
        <div className="flex-1 w-full min-w-0 pt-6">
          <main className="w-full animate-in fade-in duration-500">
            ...
            <div className="max-w-5xl mx-auto">
              ...
              {/* DRAFTS TAB (Combines Live & Online) */}
              {activeTab === 'drafts' && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 mb-16">
                  ...
                  <div className="flex justify-center mb-8">
                     <div className="bg-[#111] p-1.5 rounded-2xl border border-gray-800 flex shadow-inner w-full sm:w-auto">
                        <button 
                           onClick={() => setDraftView('online')}
                           className={`flex-1 sm:flex-none justify-center px-4 sm:px-6 py-2.5 rounded-xl text-[11px] sm:text-sm font-black uppercase tracking-widest transition-all flex items-center gap-2 ${draftView === 'online' ? 'bg-[#1b75bb] text-white shadow-md' : 'text-gray-500 hover:text-gray-300'}`}
                        >
                           <MonitorSmartphone size={16} /> Online Divisions
                        </button>
                        <button 
                           onClick={() => setDraftView('live')}
                           className={`flex-1 sm:flex-none justify-center px-4 sm:px-6 py-2.5 rounded-xl text-[11px] sm:text-sm font-black uppercase tracking-widest transition-all flex items-center gap-2 ${draftView === 'live' ? 'bg-[#1b75bb] text-white shadow-md' : 'text-gray-500 hover:text-gray-300'}`}
                        >
                  ...
                  {/* ONLINE DRAFTS CONTENT */}
                  {draftView === 'online' && (
                    <div className="animate-in fade-in duration-300">
                      
                      {/* FILTER BAR PLACEHOLDER (COMING SOON) */}
              ...
              {activeTab === 'charity' && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 mb-16">
                  ...
                  <div className="bg-gradient-to-br from-[#1a1a1a] to-[#111] rounded-3xl p-8 md:p-12 border border-gray-800 shadow-xl text-center flex flex-col items-center justify-center min-h-[350px] relative overflow-hidden">
                     ...
                     <div className="w-20 h-20 bg-[#111] border border-gray-700 rounded-full flex items-center justify-center mb-6 shadow-inner relative z-10">
                       <HeartHandshake size={32} className="text-[#f5a623]" />
                     </div>
                     ...
                     <p className="text-gray-400 text-sm md:text-base max-w-2xl leading-relaxed relative z-10">
                       Draft Night Out isn't just about winning a championship—it's about giving back. More details regarding our 2026 charity partnerships and donation initiatives will be announced here soon!
                     </p>
                  </div>
                </div>
              )}
