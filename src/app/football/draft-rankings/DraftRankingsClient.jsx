...
export default function DraftRankingsClient() {
  ...
  const processedRankings = useMemo(() => {
    ...
    const recalculated = basePlayers.map(player => {
        ...
        const P50 = Number(player['Base (P50)'] ?? player['Projected Fantasy Points']) || 0;
        const P25 = Number(player['Floor (P25)']) || P50 * 0.85; 
        const P75 = Number(player['Ceiling (P75)']) || P50 * 1.15;
        ...
        <div className="relative z-10 w-full flex flex-col md:flex-row items-start md:items-center justify-between px-6 md:px-10 gap-6 md:gap-4">
          <div className="max-w-2xl w-full">
            ...
            <p className="text-gray-300 font-medium md:text-lg">
              Season-long fantasy football draft rankings that project full-season value and outcome ranges—fully customized to match your exact league scoring rules so you know exactly who to draft.
            </p>
          </div>
          ...
          <div className="flex bg-black/40 backdrop-blur-md p-1.5 rounded-full border border-white/10 shadow-2xl shrink-0 max-w-full overflow-x-auto scrollbar-hide self-start md:self-end md:mb-8">
            <button className="px-4 py-2.5 md:px-6 md:py-3 rounded-full font-black text-[10px] md:text-xs uppercase tracking-widest transition-all bg-white text-black shadow-md whitespace-nowrap">Draft</button>
            {isRegularSeason && (
