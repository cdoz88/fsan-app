...
export default function AdsDashboard() {
  ...
  const LivePreviewAd = ({ ad }) => {
    ...
    return (
      <div className="w-full h-full rounded-2xl p-4 md:p-6 flex flex-row items-center justify-between text-left relative overflow-hidden shadow-2xl group min-h-[120px] transition-all border-2 gap-2 md:gap-4" style={{ ...bgStyles, borderColor: ad.borderColor || ad.bgColor }}>
         ...
         {/* 1. TEXT (Left Side - Gets flex-1 to demand equal space) */}
         ...
         {/* 2. IMAGE (Dead Center - Shrinks out if space gets too tight on small screens) */}
         {ad.fgImage && (
            <div className="relative z-10 hidden md:flex justify-center items-center shrink-0 px-2 md:px-4">
               <img src={ad.fgImage} className="max-h-20 md:max-h-24 w-auto max-w-[100px] md:max-w-[140px] object-contain drop-shadow-2xl hover:scale-110 transition-transform duration-300" alt="Foreground" />
            </div>
         )}

         {/* 3. BUTTON (Right Side - Gets flex-1 to perfectly balance the Text and keep image centered) */}
         <div className="relative z-10 flex-1 flex justify-end items-center shrink-0 min-w-0">
