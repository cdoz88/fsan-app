...
export default function DNOHeader({ onOpenAuthModal }) {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 w-full z-50 flex items-center justify-between px-6 py-6 bg-gradient-to-b from-black/80 via-black/40 to-transparent pointer-events-auto">
      {/* Larger DNO Logo */}
      <Link href="/dno" className="flex items-center gap-2 hover:opacity-90 transition-opacity">
        <img 
          src="https://admin.fsan.com/wp-content/uploads/2026/07/DNO-Logo_Logo.webp" 
          alt="Draft Night Out" 
          className="h-14 md:h-16 w-auto object-contain drop-shadow-[0_0_12px_rgba(0,0,0,0.8)]"
        />
      </Link>
      ...
      <div className="flex items-center gap-4 md:gap-6">
        {status === 'loading' ? (
          ...
          <>
            {/* Styled Locker Room Button with Outline and Semi-Transparent Background */}
            <Link 
              href="/dno/dashboard"
            ...
            <button 
              ...
              className="px-5 py-2 rounded-full border border-gray-700 bg-black/40 text-gray-300 text-xs font-bold uppercase tracking-widest hover:text-white hover:border-gray-500 hover:bg-black/60 transition-colors flex items-center gap-2 backdrop-blur-sm"
            >
