      ...
      <div className={`relative z-10 w-full animate-in fade-in zoom-in-95 duration-200 ${selectedItem.type === 'article' ? 'max-w-4xl' : 'max-w-6xl'} my-auto bg-[#121212] border border-gray-800 rounded-xl shadow-2xl overflow-hidden`}>
        ...
        {selectedItem.type === 'short' && (
          <div className="flex flex-col h-full max-h-[85vh]">
            ...
            {/* Top Section: Video Player & Description */}
            <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
              ...
              <div className="lg:w-1/2 p-6 sm:p-10 bg-[#121212] flex flex-col overflow-y-auto">
                ...
                <div className="flex gap-2 mb-6 pb-6 border-b border-gray-800">
                  <button onClick={() => handleShare('facebook')} className="w-10 h-10 rounded-full bg-[#4267B2]/10 text-[#4267B2] flex items-center justify-center hover:bg-[#4267B2] hover:text-white transition-colors" title="Share on Facebook"><Facebook size={16} /></button>
                  <button onClick={() => handleShare('x')} className="w-10 h-10 rounded-full bg-[#1DA1F2]/10 text-[#1DA1F2] flex items-center justify-center hover:bg-[#1DA1F2] hover:text-white transition-colors" title="Share on X"><XIcon size={16} /></button>
                  <button onClick={() => handleShare('reddit')} className="w-10 h-10 rounded-full bg-[#FF4500]/10 text-[#FF4500] flex items-center justify-center hover:bg-[#FF4500] hover:text-white transition-colors" title="Share on Reddit"><Reddit size={16} /></button>
                  <button onClick={handleCopy} className={`w-10 h-10 rounded-full ${copied ? 'bg-green-500/20 text-green-500' : 'bg-gray-700/50 text-gray-400 hover:bg-gray-600 hover:text-white'} flex items-center justify-center transition-colors`} title="Copy Link">
