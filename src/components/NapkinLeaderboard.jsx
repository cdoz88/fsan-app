            ...
            {loading ? (
              ...
              filteredTeams.map((team) => {
                ...
                if (currentWeek === 'overall') {
                   ...
                   if (team.badges?.weeklyTopScorer > 0) badges.push({ icon: <WeeklyScorerSVG />, count: team.badges.weeklyTopScorer, title: 'Weekly Top Scorer' });
                   if (team.badges?.twoHundredClub > 0) badges.push({ icon: <Club200SVG />, count: team.badges.twoHundredClub, title: '200+ Point Club' });
                } else {
                   if (team.weekly_badges?.litchAward) badges.push({ icon: <LitchSVG />, title: 'LITCH Award' });
                   if (team.weekly_badges?.weeklyTopScorer) badges.push({ icon: <WeeklyScorerSVG />, title: 'Weekly Top Scorer' });
                   if (team.weekly_badges?.twoHundredClub) badges.push({ icon: <Club200SVG />, title: '200+ Point Club' });
                }
