...
// --- ASSETS (SVGs RESTORED FOR PRODUCTION) ---
  ...
  const handleRowClick = async (teamId) => {
    ...
    try {
      ...
      if (result.success) {
         ...
         setModalError(result.data?.message || 'Failed to load team data.');
      }
    } catch (err) {
                  ...
                  <tr key={team.teamId} onClick={() => handleRowClick(team.teamId)} className="hover:bg-[#151515] transition-colors group cursor-pointer">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col"><span className="font-bold text-gray-200 text-sm group-hover:text-blue-400">{team.ownerUsername}</span><span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-0.5">{team.leagueName}</span></div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-sm flex items-center justify-center p-4">
           ...
           <div className="relative bg-[#1a1a1a] border border-gray-700 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="p-6 md:p-8 overflow-y-auto flex-1 scrollbar-hide">
                 {modalLoading ? (
                    <div className="flex flex-col items-center justify-center py-20"><Loader2 size={40} className="animate-spin text-gray-600 mb-4" /><span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Crunching Archive...</span></div>
                 ) : modalData ? (
                    <div className="flex flex-col gap-8">
                       <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div className="bg-[#111] border border-gray-800 rounded-2xl p-4 flex flex-col items-center justify-center text-center shadow-inner"><span className="text-[10px] font-black uppercase text-gray-500 mb-1">Rank</span><span className="text-2xl font-black text-white">{selectedTeam.rank}</span></div>
                          <div className="bg-[#111] border border-gray-800 rounded-2xl p-4 flex flex-col items-center justify-center text-center shadow-inner"><span className="text-[10px] font-black uppercase text-gray-500 mb-1">Total Points</span><span className="text-2xl font-black text-white">{parseFloat(selectedTeam.totalPoints).toFixed(2)}</span></div>
                          ...
                          <div className="bg-[#111] border border-gray-800 rounded-2xl p-4 flex flex-col items-center justify-center text-center shadow-inner"><span className="text-[10px] font-black uppercase text-gray-500 mb-1">Med Wins</span><span className="text-2xl font-black text-white">{Object.values(modalData.weekly_results).filter(w => w.median === 'W').length}</span></div>
                       </div>
