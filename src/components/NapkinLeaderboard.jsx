...
export default function NapkinLeaderboard({ initialTeams = [] }) {
  const [overallTeams, setOverallTeams] = useState(initialTeams);
  const [activeTeams, setActiveTeams] = useState(initialTeams);
  ...
  // Fixed Data Hydration: Unwraps "data" object properly
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const data = result.data; // Standard WP AJAX wrapping
      ...
      } catch (err) { setError('Syncing leaderboard data...'); } finally { setLoading(false); }
    };
    fetchInitialData();
  }, []);
    ...
    <div className="bg-[#1a1a1a] rounded-3xl border border-gray-800 shadow-xl overflow-hidden mt-12 flex flex-col animate-in fade-in duration-500">
                  ...
                  <tr key={team.teamId} onClick={() => handleRowClick(team.teamId)} className="hover:bg-[#151515] transition-colors group cursor-pointer">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <img src={team.ownerAvatar} className="w-10 h-10 rounded-full border border-gray-700 bg-gray-900" alt="" />
                        <div className="flex flex-col"><span className="font-bold text-gray-200 text-sm group-hover:text-blue-400 transition-colors">{team.ownerUsername}</span><span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{team.leagueName}</span></div>
                      </div>
                    </td>
      ...
      {selectedTeam && (
        <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-sm flex items-center justify-center p-4">
           ...
           <div className="relative bg-[#1a1a1a] border border-gray-700 rounded-3xl shadow-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="p-6 md:p-8 border-b border-gray-800 bg-[#111] flex items-center gap-6">
                 ...
                 <div><h2 className="text-2xl md:text-3xl font-black text-white italic tracking-tighter mb-1">{selectedTeam.ownerUsername}</h2><span className="text-xs font-bold text-gray-500 uppercase tracking-widest">{selectedTeam.leagueName}</span></div>
              </div>
              <div className="p-6 md:p-8 overflow-y-auto flex-1 scrollbar-hide">
                 {modalLoading ? ( <div className="flex flex-col items-center justify-center py-20"><Loader2 size={40} className="animate-spin text-gray-600 mb-4" /><span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Compiling Analytics...</span></div>
                 ) : modalData ? (
