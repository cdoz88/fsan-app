...
const HeroCard = ({ item, setSelectedItem }) => {
...
const SideListCard = ({ item, setSelectedItem }) => {
  const cardTheme = themes[item.sport] || themes.All;
  return (
...
const MidCard = ({ item, setSelectedItem }) => {
  const cardTheme = themes[item.sport] || themes.All;
  return (
...
const StackedCard = ({ item, setSelectedItem }) => {
  const cardTheme = themes[item.sport] || themes.All;
  return (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-stretch">
              {heroArticle && (
                <div className="xl:col-span-7 h-full">
                  <HeroCard item={heroArticle} setSelectedItem={setSelectedItem} />
                </div>
              )}
              {sideArticles.length > 0 && (
                <div className="xl:col-span-5 flex flex-col justify-between gap-4">
                  {sideArticles.map(article => (
                    <SideListCard key={article.id} item={article} setSelectedItem={setSelectedItem} />
                  ))}
                </div>
              )}
            </div>
          ...
          {midArticles.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {midArticles.map(article => (
                <MidCard key={article.id} item={article} setSelectedItem={setSelectedItem} />
              ))}
            </div>
          )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-stretch">
              <div className="xl:col-span-8 flex flex-col gap-4">
                {stackedArticles.map(article => (
                  <StackedCard key={article.id} item={article} setSelectedItem={setSelectedItem} />
                ))}
              </div>
