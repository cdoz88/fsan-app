...
export default function QandATab({
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'stream_state', 'live'), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.qa_activeChat !== undefined) setActiveChat(data.qa_activeChat);
  ...
  const handlePlayerSelect = (newPlayerId) => {
    ...
    if (type === 'swap') {
      if (side === 'sideA') {
        newLists = {
          sideA: currentSideA.map(id => id === oldPlayerId ? newPlayerId : id),
          sideB: currentSideB
        };
      } else {
        newLists = {
          sideA: currentSideA,
          sideB: currentSideB.map(id => id === oldPlayerId ? newPlayerId : id)
        };
      }
    } else if (type === 'add') {
      if (side === 'sideA') {
        newLists = {
          sideA: [...currentSideA, newPlayerId],
          sideB: currentSideB
        };
      } else {
        newLists = {
          sideA: currentSideA,
          sideB: [...currentSideB, newPlayerId]
        };
      }
    }
  ...
  const renderDNOLandscapeCard = (playerId, side = 'sideA') => {
        ...
        >
          {isGrayedOut && (
            <div className="absolute inset-0 z-30 bg-black/60 backdrop-blur-[2px] flex items-center justify-center pointer-events-none animate-in fade-in duration-200">
              <div className="bg-red-600 text-white rounded-full p-2.5 shadow-2xl border-2 border-white/90">
          ...
          <div className="absolute inset-0 z-40 bg-black/80 backdrop-blur-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-2 p-2">
            <button 
              onClick={(e) => togglePlayerDisabled(playerId, e)}
              className="p-2.5 bg-red-600 hover:bg-red-500 text-white rounded-xl shadow-lg transition-transform hover:scale-110 flex items-center justify-center"
              title={isGrayedOut ? "Restore Pick" : "Sit Pick"}
            >
              <X size={18} strokeWidth={3} />
            </button>
            <button 
              onClick={(e) => { e.stopPropagation(); setPlayerSearch({ type: 'swap', oldPlayerId: playerId, side }); }}
              className="p-2.5 bg-zinc-800 hover:bg-zinc-700 text-white border border-zinc-600 rounded-xl shadow-lg transition-transform hover:scale-110 flex items-center justify-center"
              title="Swap Pick"
            >
              <RefreshCw size={18} />
            </button>
    ...
    return (
      <div 
        ...
        `}
        style={{ 
          background: `linear-gradient(180deg, ${tColors.primary}90 0%, ${tColors.secondary}95 65%, #000000 100%)` 
        }}
      >
        {isGrayedOut && (
          <div className="absolute inset-0 z-30 bg-black/60 backdrop-blur-[2px] flex items-center justify-center pointer-events-none animate-in fade-in duration-200">
        ...
        <div className="absolute inset-0 z-40 bg-black/80 backdrop-blur-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-2 p-2">
          <button 
            onClick={(e) => togglePlayerDisabled(playerId, e)}
            className="p-2.5 bg-red-600 hover:bg-red-500 text-white rounded-xl shadow-lg transition-transform hover:scale-110 flex items-center justify-center"
            title={isGrayedOut ? "Restore Player" : "Sit Player"}
          >
            <X size={18} strokeWidth={3} />
          </button>
          <button 
            onClick={(e) => { e.stopPropagation(); setInfoPlayerId(playerId); }}
            className="p-2.5 bg-[#1b75bb] hover:bg-[#155e96] text-white rounded-xl shadow-lg transition-transform hover:scale-110 flex items-center justify-center"
        ...
        <div className="relative z-20 px-2 pb-2 pt-1 text-center pointer-events-none w-full truncate">
          <div className={`text-[10px] font-bold tracking-widest uppercase leading-none ${cardStyle.text} drop-shadow-md mb-0.5`}>
            {firstName}
          </div>
          <div className="text-xl font-black text-white tracking-tight truncate w-full drop-shadow-lg leading-none uppercase">
            {lastName}
          </div>
        </div>
      </div>
        ...
        {priorityQueue.length > 0 && (
          <div className="bg-[#141418] border-b border-amber-500/30 p-3 shadow-md flex flex-col gap-2 relative overflow-hidden shrink-0">
            ...
            <div className="text-[10px] font-black text-amber-500 uppercase tracking-widest flex items-center gap-2 px-1">
              ...
              Priority Line Jumper Waiting ({priorityQueue.length})
            </div>
        ...
        <div className="p-2 px-3 border-b border-zinc-800/60 bg-black/40 flex justify-between items-center z-10 min-h-[36px] shrink-0">
          {activeChat ? (
            <button 
              ...
              title="Reset Graphic to Original"
            >
              <RotateCcw size={16} />
            </button>
          ) : <div />}
      ...
      <div className="w-80 lg:w-88 bg-[#0e0e11] border border-zinc-800 rounded-2xl flex flex-col shadow-2xl flex-shrink-0 min-h-0">
        ...
        <div className="flex bg-[#141418] border-b border-zinc-800 rounded-t-2xl overflow-hidden p-1.5 gap-1.5 shrink-0">
          <button
            ...
            className={`flex-1 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 ${activeSidebarTab === 'live' ? 'bg-zinc-800 text-white shadow-md' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50'}`}
          >
            Live Chat
            <span className={`px-1.5 py-0.5 rounded-md text-[9px] ${activeSidebarTab === 'live' ? 'bg-zinc-600 text-white' : 'bg-[#0e0e11] text-zinc-600'}`}>{regularChats.length}</span>
          </button>
          
          <button
            onClick={() => setActiveSidebarTab('super')}
            className={`flex-1 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 ${activeSidebarTab === 'super' ? 'bg-emerald-600 text-white shadow-md' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50'}`}
          >
            Super Chats
            <span className={`px-1.5 py-0.5 rounded-md text-[9px] ${activeSidebarTab === 'super' ? 'bg-emerald-800 text-white' : 'bg-[#0e0e11] text-zinc-600'}`}>{superChats.length}</span>
          </button>
