const decodeWP = (text) => {
  ...
  export const formatPost = (post) => {
      ...
      // 1. Check if slugs contain football keywords
      // 2. NEW: Specifically check if "article" is the slug (FSAN subcategory logic)
      // 3. Check title as backup
      if (
    ...
    export const fetchPosts = async (activeSport, targetType, currentPage = 1) => {
      try {
        ...
        if (targetType === 'all') {
          const endpoints = [
            `https://fsan.com/wp-json/fsan/v1/feed?per_page=15&page=${currentPage}&sport=${activeSport}&type=articles`,
            `https://fsan.com/wp-json/fsan/v1/feed?per_page=15&page=${currentPage}&sport=${activeSport}&type=videos`,
            `https://fsan.com/wp-json/fsan/v1/feed?per_page=15&page=${currentPage}&sport=${activeSport}&type=podcasts`,
            `https://fsan.com/wp-json/fsan/v1/feed?per_page=15&page=${currentPage}&sport=${activeSport}&type=shorts`,
            `https://fsan.com/wp-json/fsan/v1/feed?per_page=20&page=${currentPage}&sport=${activeSport}&type=shows`
          ];
          ...
          const res = await fetch(`https://fsan.com/wp-json/fsan/v1/feed?per_page=24&page=${currentPage}&sport=${activeSport}&type=${targetType}`, fetchOptions);
