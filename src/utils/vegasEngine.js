...
async function fetchLiveVegasData() {
  ...
  // If API key is missing, drop directly into offseason mock view
  if (!API_KEY) {
    return { rankings: MOCK_DRAFT_RANKINGS, mode: 'offseason', error: 'THE_ODDS_API_KEY is missing from Vercel.' };
  }
  ...
  try {
    ...
    return { rankings: MOCK_DRAFT_RANKINGS, mode: 'offseason', error: error.message };
  }
}

// Next.js will cache this function and only rerun it when the Cron job tells it to
